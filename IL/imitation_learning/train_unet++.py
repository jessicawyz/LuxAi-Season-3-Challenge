...
MODEL_NAME = "imitation_learning/unet++"
...
class NestedUNet(nn.Module):
    def __init__(self, n_channels=N_CHANNELS, n_classes=N_CLASSES, n_global=N_GLOBAL, bilinear=True):
        ...
        self.global_fc = nn.Linear(n_global * 3 * 3, 512)

        # ---- Decoder / Nested Skip Connections ----
        # Up blocks: input = sum of channels from upsample + skip
        self.up3_0 = Up(512 + 256, 256, bilinear)  # x3_0 + x2_0
        self.up2_0 = Up(256 + 128, 128, bilinear)  # x2_1 + x1_0
        self.up1_0 = Up(128 + 64, 64, bilinear)    # x1_1 + x0_0

        # Nested convolutions (dense skip connections)
        self.conv0_1 = DoubleConv(64 + 64, 64)      # x0_0 + x1_0
        self.conv0_2 = DoubleConv(64*2 + 64, 64)    # x0_0 + conv0_1 + x1_1
        self.conv0_3 = DoubleConv(64*3 + 64, 64)    # x0_0 + conv0_1 + conv0_2 + x1_2

        # ---- Output ----
        self.outc = OutConv(64, n_classes)

    def forward(self, x, gf=None):
        # ---- Encoder ----
        x0_0 = self.inc(x)       
        ...
        if gf is not None:
            ...
            gf_flat = gf.view(batch_size, -1)       # flatten everything except batch
            gf_embed = self.global_fc(gf_flat)      # (batch, 512)
            gf_embed = gf_embed.unsqueeze(-1).unsqueeze(-1)  # (batch, 512, 1, 1)
            x3_0 = x3_0 + gf_embed  

        # ---- Decoder ----
        x2_1 = self.up3_0(x3_0, x2_0)       # channels: 256
        x1_1 = self.up2_0(x2_1, x1_0)       # channels: 128
        x0_1 = self.up1_0(x1_1, x0_0)       # channels: 64

        # Nested skip connections
        x0_1 = self.conv0_1(torch.cat([x0_0, x0_1], dim=1))           # 64+64 = 128 → 64
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, x0_1], dim=1))      # 64+64+64 = 192 → 64
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, x0_1], dim=1)) # 64+64+64+64 = 256 → 64
