...
try:
    ...
    # Fallback to kits path if running from different location
    from kits.python.ppo.agent_mappo import Agent

# Import lux.kit utilities
parent_dir = os.path.dirname(dirname) if dirname else os.path.dirname(os.getcwd())
sys.path.append(parent_dir)
try:
    from lux.kit import from_json
except ImportError:
    # Fallback: create a simple from_json function
    def from_json(obj):
        return obj
...
def agent_fn(observation, configurations):
    ...
    # Convert JAX array to numpy array then to list
    if hasattr(actions, '__array__'):
