...
class Agent:
    ...
    def _create_kaggle_action_array(self, actions_dict, sap_dict, unit_info):
        """Create action array for Kaggle - returns JAX array for compatibility"""
        action_array = jnp.zeros((self.max_units, 3), dtype=jnp.int32)
        ...
        for unit in unit_info:
            ...
            if action_val == 0:
                action_array = action_array.at[unit_id].set(jnp.array([0, 0, 0], dtype=jnp.int32))
            elif 1 <= action_val <= 4:
                action_array = action_array.at[unit_id].set(jnp.array([action_val, 0, 0], dtype=jnp.int32))
            elif action_val == 5:
                dx, dy = sap_dict.get(unit_id, (0, 0))
                action_array = action_array.at[unit_id].set(jnp.array([5, dx, dy], dtype=jnp.int32))
